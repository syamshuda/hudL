...
// Keamanan: Pastikan hanya GURU yang bisa akses
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_profil'])) {
    ...
    if (!empty($nama_lengkap) && !empty($email)) {
        ...
        $stmt_cek_email->bind_param("si", $email, $id_guru);
        ...
        if ($result_cek_email->num_rows > 0) {
            ...
            $stmt->bind_param("sssi", $nama_lengkap, $email, $no_hp, $id_guru);
            if ($stmt->execute()) {
    ...
    if (!empty($password_lama) && !empty($password_baru) && !empty($konfirmasi_password)) {
        if ($password_baru !== $konfirmasi_password) {
            ...
            if (hash('sha256', $password_lama) === $user['password']) {
                ...
                $stmt_update->bind_param("si", $hashed_password_baru, $id_guru);
...
$stmt_guru = $koneksi->prepare("SELECT nama_lengkap, email, no_hp FROM pengguna WHERE id = ?");
    ...
    <div class="col-md-6">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                ...
                <form action="guru_profil.php" method="POST">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="tel" class="form-control" id="no_hp" name="no_hp" value="<?php echo htmlspecialchars($guru['no_hp'] ?? ''); ?>">
                    </div>
